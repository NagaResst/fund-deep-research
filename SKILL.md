...
├── raw/
│   ├── fund_enhanced.json          ← parallel_data_collection 输出
│   ├── risk_metrics.json
│   ├── holdings.json
│   ├── quarterly.json
...
│   └── nav_daily.json
├── analysis/
│   ├── inflection_points.json      ← Step 5A 拐点脚本输出
...
```bash
python3 skills/fund-deep-research/scripts/precheck.py <基金代码>
```
...
**必须优先使用并行脚本**，一条命令同时运行5个脚本，大幅节省时间：
...
# ✅ 一键并行执行全部 8 个数据脚本
python3 skills/fund-deep-research/scripts/parallel_data_collection.py ${CODE} --output-dir ${TMP}
```

并行脚本包含（共8个）：
- **并发执行**：fetch_fund_enhanced、fetch_risk_metrics、analyze_holdings、analyze_quarterly_performance、fetch_manager_info、fetch_nav_daily
- **依赖 fund_enhanced 后串行**：scan_institutional_risk、check_blacklist

输出文件：`fund_enhanced.json`、`risk_metrics.json`、`holdings.json`、`quarterly.json`、`manager_info.json`、`nav_daily.json`、`institutional_risk.json`、`blacklist.json`
...
```bash
python3 skills/fund-deep-research/scripts/check_data_integrity.py <基金代码>
```
...
使用 `/tmp/fund_research_{code}/raw/nav_daily.json`，运行以下脚本，结果保存到 `/tmp/fund_research_{code}/analysis/inflection_points.json`：

```bash
python3 skills/fund-deep-research/scripts/analyze_inflection_points.py <基金代码>
```
...
> ✅ 脚本运行完毕后**无需在对话中输出拐点列表**，后续所有深度搜索（B/C/D/E/F）通过 `read_file` 读取 `/tmp/fund_research_{code}/analysis/inflection_points.json` 获取数据，不依赖记忆。

#### B. 全量季度数据（直接使用 Step 1 已有输出）

> `analyze_quarterly_performance.py` 已在 Step 1 运行完毕，**无需重跑**。
> 通过 `read_file` 读取 `/tmp/fund_research_{code}/raw/quarterly.json` 作为逐季复盘骨架（含每季末净值、前十大重仓股及占比）。

#### C. 逐季深度复盘：盈亏-持仓-通告三维绑定（核心工作）
...
|---------|------|----------------------------------------|
| 1 | **第一章** 基金基本信息 | `raw/fund_enhanced.json` · `raw/manager_info.json` |
| 2 | **第三章** 基金发展历史 | `analysis/inflection_points.json` · `search_log.md`（grep `[Step5-C]` `[Step5-G]` 段） |
| 3 | **第四章** 基金经理深度分析 | `raw/manager_info.json` · `search_log.md`（grep `[Step5-H]` 段） |
| 4 | **第五章** 基金公司合规评估 | `raw/institutional_risk.json` · `raw/blacklist.json` · `search_log.md`（grep `[Step5-I]` 段） |
| 5 | **第六章** 持仓分析 | `raw/holdings.json` |
| 6 | **第七章** 风险指标 | `raw/risk_metrics.json` |
| 7 | **第八章** 行业与政策背景 | `search_log.md`（grep `[Step5-E]` 段） |
| 8 | **第九章** 历史业绩分析 | `raw/quarterly.json` · `search_log.md`（grep `[Step5-D]` `[Step5-F]` 段） |
